...
*Setup instructions for a Gitian build of Coin2College Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Coin2College
...
- [Building Coin2College Core](#building-coin2college-core)
...
For example, to connect as `root` from a Linux command prompt use
    ...
    Debian GNU/Linux comes with COIN2COLLEGELY NO WARRANTY, to the extent
...
Clone the git repositories for Coin2College Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/coin2collegecrypto/coin2college
```
...
in the Coin2College Core repository under 'Fetch and build inputs' to install sources which require
...
To build Coin2College Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Coin2College Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/coin2college/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/coin2collegecrypto/coin2college
...
./bin/gbuild --commit coin2college=${COMMIT} --url coin2college=${URL} ../coin2college/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit coin2college=${COMMIT} --url coin2college=${URL} ../coin2college/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit coin2college=${COMMIT} --url coin2college=${URL} ../coin2college/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the coin2college git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../coin2college/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/coin2collegecrypto/coin2college-detached-sigs

BTCPATH=/some/root/path/coin2college.git
SIGPATH=/some/root/path/coin2college-detached-sigs.git

./bin/gbuild --url coin2college=${BTCPATH},signature=${SIGPATH} ../coin2college/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/coin2college-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/coin2college-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/coin2college-osx-build.assert
```
