...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Coin2College users should be well used to small numbers with many decimal places by now :)
                ...
                {
                    {
                        ...
                        {
                            // socket closed gcoin2collegefully
                            if (!pnode->fDisconnect)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Coin2College Core " + FormatFullVersion();
...
void MapPort(bool fUseUPnP)
{
    ...
    if (fUseUPnP)
    {
        ...
        upnp_thread = new boost::thread(boost::bind(&Tcoin2collegeThread<void (*)()>, "upnp", &ThreadMapPort));
    }
    else if (upnp_thread) {
        upnp_thread->interrupt();
        upnp_thread->join();
...
bool CConnman::BindListenPort(const CService &addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Coin2College Core is probably already running."), addrBind.ToString());
        else
...
bool CConnman::Start(CScheduler& scheduler, std::string& strNodeError, Options connOptions)
{
    ...
    threadSocketHandler = std::thread(&Tcoin2collegeThread<std::function<void()> >, "net", std::function<void()>(std::bind(&CConnman::ThreadSocketHandler, this)));

    if (!GetBoolArg("-dnsseed", true))
        LogPrintf("DNS seeding disabled\n");
    else
        threadDNSAddressSeed = std::thread(&Tcoin2collegeThread<std::function<void()> >, "dnsseed", std::function<void()>(std::bind(&CConnman::ThreadDNSAddressSeed, this)));

    // Initiate outbound connections from -addnode
    threadOpenAddedConnections = std::thread(&Tcoin2collegeThread<std::function<void()> >, "addcon", std::function<void()>(std::bind(&CConnman::ThreadOpenAddedConnections, this)));

    // Initiate outbound connections
    threadOpenConnections = std::thread(&Tcoin2collegeThread<std::function<void()> >, "opencon", std::function<void()>(std::bind(&CConnman::ThreadOpenConnections, this)));

    // Initiate masternode connections
    threadMnbRequestConnections = std::thread(&Tcoin2collegeThread<std::function<void()> >, "mnbcon", std::function<void()>(std::bind(&CConnman::ThreadMnbRequestConnections, this)));

    // Process messages
    threadMessageHandler = std::thread(&Tcoin2collegeThread<std::function<void()> >, "msghand", std::function<void()>(std::bind(&CConnman::ThreadMessageHandler, this)));
