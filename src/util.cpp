...
const char * const BITCOIN_CONF_FILENAME = "coin2college.conf";
const char * const BITCOIN_PID_FILENAME = "coin2colleged.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "coin2college" is a composite category enabling all Coin2College-related debug output
            if(ptrCategory->count(string("coin2college"))) {
...
#else
    const char* pszModule = "coin2college";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Coin2CollegeCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Coin2CollegeCore
    // Mac: ~/Library/Application Support/Coin2CollegeCore
    // Unix: ~/.coin2collegecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Coin2CollegeCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Coin2CollegeCore";
#else
    // Unix
    return pathRet / ".coin2collegecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty coin2college.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override coin2college.conf
