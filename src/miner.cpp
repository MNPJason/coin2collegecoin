...
// ***TODO*** ScanHash is not yet used in Coin2College
//
...
{
    LogPrintf("Coin2CollegeMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coin2college-miner");
    ...
    try {
        ...
        while (true) {
            ...
            // Create new block
            //
            LogPrintf("Coin2CollegeMiner -- Creating a new block\n");
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Coin2CollegeMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Coin2CollegeMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("Coin2CollegeMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("Coin2CollegeMiner -- runtime error: %s\n", e.what());
        return;
    }
}
