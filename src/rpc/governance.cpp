...
//#define ENABLE_COIN2COLLEGE_DEBUG
...
UniValue gobject(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in coin2college.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("coin2college.conf", statusObj));
    ...
    if(strCommand == "list" || strCommand == "diff")
    {
        ...
        BOOST_FOREACH(CGovernanceObject* pGovObj, objs)
        {
            ...
            bObj.push_back(Pair("Coin2CollegeYesCount",  pGovObj->GetCoin2CollegeYesCount(VOTE_SIGNAL_FUNDING)));
    ...
    if(strCommand == "get")
    {
        ...
        objFundingResult.push_back(Pair("Coin2CollegeYesCount",  pGovObj->GetCoin2CollegeYesCount(VOTE_SIGNAL_FUNDING)));
        ...
        objValid.push_back(Pair("Coin2CollegeYesCount",  pGovObj->GetCoin2CollegeYesCount(VOTE_SIGNAL_VALID)));
        ...
        objDelete.push_back(Pair("Coin2CollegeYesCount",  pGovObj->GetCoin2CollegeYesCount(VOTE_SIGNAL_DELETE)));
        ...
        objEndorsed.push_back(Pair("Coin2CollegeYesCount",  pGovObj->GetCoin2CollegeYesCount(VOTE_SIGNAL_ENDORSED)));
...
UniValue getgovernanceinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0) {
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"governanceminquorum\": xxxxx,           (numeric) the coin2college minimum number of votes needed to trigger a governance action\n"
...
UniValue getsuperblockbudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nReturns the coin2college maximum sum of superblock payments allowed.\n"
            "\nArguments:\n"
            "1. index         (numeric, required) The block index\n"
            "\nResult:\n"
            "n                (numeric) The coin2college maximum sum of superblock payments allowed, in " + CURRENCY_UNIT + "\n"
            "\nExamples:\n"
