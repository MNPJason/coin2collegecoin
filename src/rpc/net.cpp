...
UniValue getpeerinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "    \"subver\": \"/Coin2College Core:x.x.x/\",  (string) The string version\n"
...
UniValue getaddednodeinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:8800\",  (string) The coin2college server IP and port we're connected to\n"
...
UniValue getnetworkinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"subversion\": \"/Coin2College Core:x.x.x/\",     (string) the server subversion string\n"
...
UniValue setban(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 ||
        ...
        throw runtime_error(
                            "setban \"ip(/netmask)\" \"add|remove\" (bantime) (coin2college)\n"
                            "\nAttempts add or remove a IP/Subnet from the banned list.\n"
                            "\nArguments:\n"
                            "1. \"ip(/netmask)\" (string, required) The IP/Subnet (see getpeerinfo for nodes ip) with a optional netmask (default is /32 = single ip)\n"
                            "2. \"command\"      (string, required) 'add' to add a IP/Subnet to the list, 'remove' to remove a IP/Subnet from the list\n"
                            "3. \"bantime\"      (numeric, optional) time in seconds how long (or until when if [coin2college] is set) the ip is banned (0 or empty means using the default time of 24h which can also be overwritten by the -bantime startup argument)\n"
                            "4. \"coin2college\"     (boolean, optional) If set, the bantime must be a coin2college timestamp in seconds since epoch (Jan 1 1970 GMT)\n"
                            "\nExamples:\n"
    ...
    if (strCommand == "add")
    {
        ...
        bool coin2college = false;
        if (params.size() == 4 && params[3].isTrue())
            coin2college = true;

        isSubnet ? g_connman->Ban(subNet, BanReasonManuallyAdded, banTime, coin2college) : g_connman->Ban(netAddr, BanReasonManuallyAdded, banTime, coin2college);
    }
    else if(strCommand == "remove")
    {
