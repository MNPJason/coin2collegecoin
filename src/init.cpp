...
#include "config/coin2college-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("coin2college-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "coin2college (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-printtoconsole", _("Send tcoin2college/debug info to console instead of debug.log file"));
    strUsage += HelpMessageOpt("-printtodebuglog", strprintf(_("Send tcoin2college/debug info to debug.log file (default: %u)"), 1));
    if (showDebug)
    {
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Coin2College specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N C2C anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The Coin2College Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("coin2college-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that Coin2College Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Coin2College Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize Coin2College Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Coin2College Core is shutting down."));
...
#endif
    // Make sure only a single Coin2College Core process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Coin2College Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Coin2College Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    threadGroup.create_thread(boost::bind(&Tcoin2collegeThread<CScheduler::Function>, "scheduler", serviceLoop));
    ...
    // Note that we coin2collegely cannot open any actual connections
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Coin2College Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Coin2College Core to complete") << "\n";
    ...
    // ********************************************************* Step 11c: update block tip in Coin2College modules
    ...
    // ********************************************************* Step 11d: start coin2college-ps-<smth> threads
