...
bool CGovernanceObject::IsValidLocally(std::string& strError, bool& fMissingMasternode, bool& fMissingConfirmations, bool fCheckCollateral)
{
    ...
    // IF COIN2COLLEGE NO COUNT (NO-YES VALID VOTES) IS MORE THAN 10% OF THE NETWORK MASTERNODES, OBJ IS INVALID
        ...
        if((nObjectType == GOVERNANCE_OBJECT_TRIGGER) || (nObjectType == GOVERNANCE_OBJECT_WATCHDOG)) {
            ...
            if(!mnodeman.GetMasternodeInfo(vinMasternode.prevout, infoMn)) {
                ...
                if (err == CMasternode::COLLATERAL_OK) {
                    ...
                    strError = "Masternode UTXO should have 1000 C2C, missing masternode=" + strOutpoint + "\n";
                }
...
int CGovernanceObject::GetCoin2CollegeYesCount(vote_signal_enum_t eVoteSignalIn) const
{
    return GetYesCount(eVoteSignalIn) - GetNoCount(eVoteSignalIn);
}

int CGovernanceObject::GetCoin2CollegeNoCount(vote_signal_enum_t eVoteSignalIn) const
{
...
{
    return CountMatchingVotes(eVoteSignalIn, VOTE_OUTCOME_C2CTAIN);
}
...
void CGovernanceObject::UpdateSentinelVariables()
{
    ...
    if(GetCoin2CollegeYesCount(VOTE_SIGNAL_FUNDING) >= nAbsVoteReq) fCachedFunding = true;
    if((GetCoin2CollegeYesCount(VOTE_SIGNAL_DELETE) >= nAbsDeleteReq) && !fCachedDelete) {
        fCachedDelete = true;
        if(nDeletionTime == 0) {
            nDeletionTime = GetAdjustedTime();
        }
    }
    if(GetCoin2CollegeYesCount(VOTE_SIGNAL_ENDORSED) >= nAbsVoteReq) fCachedEndorsed = true;

    if(GetCoin2CollegeNoCount(VOTE_SIGNAL_VALID) >= nAbsVoteReq) fCachedValid = false;
}
