...
pkgdatadir = $(datadir)/coin2collegecore
pkgincludedir = $(includedir)/coin2collegecore
pkglibdir = $(libdir)/coin2collegecore
pkglibexecdir = $(libexecdir)/coin2collegecore
...
CONFIG_HEADER = $(top_builddir)/src/config/coin2college-config.h
CONFIG_CLEAN_FILES = libcoin2collegeconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libcoin2collegeconsensus.pc.in \
	...
	$(top_srcdir)/src/config/coin2college-config.h.in \
...
ACLOCAL = ${SHELL} /media/sf_jerry/coindev/coin2college/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /media/sf_jerry/coindev/coin2college/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /media/sf_jerry/coindev/coin2college/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /media/sf_jerry/coindev/coin2college/build-aux/missing automake-1.15
AWK = mawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /media/sf_jerry/coindev/coin2college/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = coin2collegecore
PACKAGE_BUGREPORT = https://github.com/coin2collegecrypto/coin2collegecoin/issues
PACKAGE_NAME = Coin2College Core
PACKAGE_STRING = Coin2College Core 0.12.2
PACKAGE_TARNAME = coin2collegecore
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /media/sf_jerry/coindev/coin2college
abs_srcdir = /media/sf_jerry/coindev/coin2college
abs_top_builddir = /media/sf_jerry/coindev/coin2college
abs_top_srcdir = /media/sf_jerry/coindev/coin2college
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /media/sf_jerry/coindev/coin2college/build-aux/install-sh
...
pkgconfig_DATA = libcoin2collegeconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/coin2colleged$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/coin2college-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/coin2college-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = Coin2College-Qt.app
OSX_DMG = Coin2College-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_coin2college_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_coin2college_coverage.info test_coin2college.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Coin2College-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/coin2college-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/coin2college-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/coin2college-config.h
$(top_srcdir)/src/config/coin2college-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/coin2college-config.h src/config/stamp-h1
libcoin2collegeconsensus.pc: $(top_builddir)/config.status $(srcdir)/libcoin2collegeconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/Coin2College-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Coin2College-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "Coin2College-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Coin2College-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_coin2college.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_coin2college -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_coin2college_filtered.info: test_coin2college.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_coin2college_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_coin2college_filtered.info
...
#test_coin2college_coverage.info: baseline_filtered_combined.info test_coin2college_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_coin2college_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_coin2college_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_coin2college_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_coin2college.coverage/.dirstamp:  test_coin2college_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_coin2college.coverage/.dirstamp total.coverage/.dirstamp

# Coin2College: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_coin2college.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
